...
export default function App() {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/system-data/")
      .then(res => res.json())
  ...
  const fetchAppointmentsFromDB = async (userId) => {
    try {
      const response = await fetch(`http://localhost:8080/appointments/${userId}`);
  ...
  const fetchDoctorPatients = async (doctorName) => {
    try {
      const response = await fetch(`http://localhost:8080/doctor-appointments/Dr. ${doctorName}`);
      const data = await response.json();
      if (response.ok) {
  ...
  const handleCancelAppointment = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/appointments/${id}`, { method: 'DELETE' });
      if (response.ok) {
